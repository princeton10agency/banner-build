...
function validateClickTagSetup(html) {
  ...
  const hasWindowClickTagAssignment = /\bwindow\s*\.\s*clickTag\s*=\s*clickTag\b/.test(html)

  if (!hasVarClickTag && !hasWindowClickTagAssignment) {
    return 'Missing both "var clickTag = ..." and "window.clickTag = clickTag"'
  }
  if (!hasVarClickTag) {
    return 'Missing required "var clickTag = ..." declaration'
  }
  if (!hasWindowClickTagAssignment) {
    return 'Missing required "window.clickTag = clickTag" assignment'
  }
...
function copyVariantStaticAsset(config, outDir, variant) {
  const staticSource = resolveVariantStaticSource(config, variant)
  fs.copyFileSync(staticSource, path.join(outDir, resolveStaticOutputName(config)))
}
...
function createStaticsZip(config, variants) {
  const staticEntries = []
  const projectName = path.basename(ROOT)
  ...
  for (const variant of Array.isArray(variants) ? variants : []) {
    ...
    staticEntries.push({
      ...
      outputPrefix: variant.outDirName
    })
  }
  ...
  for (const size of getImageOnlyStaticSizes(config)) {
    staticEntries.push({
      ...
      outputPrefix: `${projectName}-${(size && size.id) || 'image'}`
    })
  }
  ...
  try {
    for (const entry of staticEntries) {
      ...
      if (usedNames.has(outputName) && path.resolve(usedNames.get(outputName)) !== path.resolve(src)) {
        outputName = `${entry.outputPrefix}-${requestedName}`
      }
      if (usedNames.has(outputName) && path.resolve(usedNames.get(outputName)) === path.resolve(src)) {
        continue
      }
      usedNames.set(outputName, src)
      fs.copyFileSync(src, path.join(stageDir, outputName))
    }
...
function build() {
  ...
  for (const variant of variants) {
    ...
    copyVariantStaticAsset(config, variant.outDir, variant)
    createVariantZip(variant)

    console.log(`Built ${path.relative(ROOT, variant.outDir)}`)
  }

  const staticsZipName = createStaticsZip(config, variants)
...
if (process.argv.includes('--lint-assets')) {
  const ok = lintAssets()
  if (!ok) {
    process.exitCode = 1
  }
