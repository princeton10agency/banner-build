...
const ISI_SCREENSHOT_STAGE_DIR = '.isi-screenshots-tmp'
...
function resolveIsiScreenshotsConfig(config) {
  ...
  const customHideSelectors = Array.isArray(block.hide_selectors)
    ? block.hide_selectors.map((selector) => String(selector || '').trim()).filter(Boolean)
    : []
...
function getIsiScreenshotInitScript() {
  return () => {
    ...
    const wrapCtor = (Ctor) => {
      ...
      try {
        ...
        // Some constructors do not allow prototype reassignment. Ignore that here.
      }
...
async function expandIsiForScreenshot(page, settings) {
  return page.evaluate((input) => {
    ...
    return {
      width,
      height: Math.ceil(captureEl.getBoundingClientRect().height)
    }
  }, settings)
}

async function captureIsiScreenshot(page, variant, settings, outputPath) {
  await page.setViewportSize({
    width: Math.max(1, Math.ceil(Number(variant.size.width) || 1)),
    height: Math.max(600, Math.ceil(Number(variant.size.height) || 600))
  })

  await page.goto(pathToFileURL(path.join(variant.outDir, 'index.html')).href, {
    waitUntil: 'load'
  })
...
function getBuildVariants(config) {
  ...
  for (const alt of alts) {
    ...
    for (const size of sizes) {
      ...
      const outDirName = hasAlt ? `${projectName}-${altId}-${sizeId}` : `${projectName}-${sizeId}`
...
function buildDistIndex(variants, staticsZipName, isiScreenshotsZipName) {
...
<html lang="en">
  ...
  <body>
    <header>
      ...
      ${isiScreenshotsZipName ? `<a class="download-link" href="${isiScreenshotsZipName}" download>Download ISI Screenshots</a>` : ''}
    </header>
    <main class="grid">
${rows}
    </main>
...
function logIsiScreenshotsSkipped(reason) {
  console.log(`Skipping ISI screenshots for ${path.basename(ROOT)}: ${reason}`)
}

async function createIsiScreenshotsZip(config, variants) {
  const screenshotConfig = resolveIsiScreenshotsConfig(config)
  if (!screenshotConfig.enabled) {
    logIsiScreenshotsSkipped('isi_screenshots is missing or disabled')
    return null
  }
  ...
  try {
    ...
    try {
      ...
      throw new Error('ISI screenshot capture requires the "playwright" or "playwright-core" package to be installed')
    }
  }

  const projectName = path.basename(ROOT)
  const zipName = `${projectName}-isi-screenshots.zip`
  const zipPath = path.join(DIST_DIR, zipName)
  const stageDir = path.join(DIST_DIR, ISI_SCREENSHOT_STAGE_DIR)
  ...
  try {
    ...
    throw new Error([
      'Unable to launch Chromium for ISI screenshots.',
  ...
  try {
    for (const variant of Array.isArray(variants) ? variants : []) {
      ...
      try {
        await page.addInitScript(getIsiScreenshotInitScript())
        const outputPath = path.join(stageDir, `${variant.outDirName}.png`)
        await captureIsiScreenshot(page, variant, screenshotConfig, outputPath)
        console.log(`Captured ISI screenshot for ${path.relative(ROOT, variant.outDir)}`)
      } finally {
...
async function build(options = {}) {
  if (!fs.existsSync(CONFIG_PATH)) {
    if (options.isiScreenshots) {
      logIsiScreenshotsSkipped('creative.config.json not found')
      return
    }
  ...
  const isiScreenshotsZipName = options.isiScreenshots
    ? (screenshotConfig.enabled
      ? await createIsiScreenshotsZip(config, variants)
      : (logIsiScreenshotsSkipped('isi_screenshots is missing or disabled'), null))
    : null
  writeFile(path.join(DIST_DIR, 'index.html'), buildDistIndex(variants, staticsZipName, isiScreenshotsZipName))
...
async function runBuildSafe(options = {}) {
  try {
    await build(options)
  } catch (error) {
...
async function main(argv = process.argv.slice(2)) {
  const isiScreenshots = argv.includes('--isi-screenshots')
  ...
  if (argv.includes('--watch')) {
    watch({ isiScreenshots })
    return
  }

  await build({ isiScreenshots })
}
