...
The published package intentionally includes only runtime files:

- `bin/`
- `lib/`

Tests and local fixtures are not included in the npm package.

## Automated publishing

This package can be published automatically from GitHub Actions when a version tag is pushed.
...
- checks that the tag version matches `package.json`
- runs `npm ci`
- runs `npm test`
- publishes to npm
...
If `package.json` becomes `1.0.1`, the workflow expects the pushed tag to be:
...
The workflow is configured for npm trusted publishing through GitHub Actions OIDC instead of a long-lived npm token.

You still need to configure the trusted publisher on npm for this package:

- package: `@p10agency/banner-build`
- GitHub organization or user: `princeton10agency`
- repository: `banner-build`
...
Once the trusted publisher is configured on npm, pushing a matching `v*` tag will publish the package automatically.
